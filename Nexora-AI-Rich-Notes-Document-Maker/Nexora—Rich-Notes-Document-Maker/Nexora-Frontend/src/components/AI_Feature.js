...
export const Bubble_Menu = ref([
    ...
    { action: "Beautify", loading: false, content: "Just Correct the grammar, spelling, punctuation, and sentence structure while preserving the original meaning and tone. Then Beautify and Format the content: clean structure, consistent spacing, readable headings, lists, and markdown. STRICTLY PRESERVE important original structure: if the content is a table, return a table (never convert a table into a list, paragraphs, or any other format); if it is a list, keep a list; if it is headings/sections, keep those headings/sections; if it is code, keep code. Do not add new ideas. NO MORE!" },
    ...
    { action: "Image", loading: false, content: "Write IMAGE TITLE and Generate image if content is visual, otherwise return same content with note." },
    ...
    { action: "Execute", loading: false, content: `Treat the selected text as an instruction / request / question and fully execute / answer / perform what it is asking. Do not rewrite or improve the instruction itself. Just do what the selected content is telling you to do and return the final result.` },
...
export let mistral_api_key = import.meta.env.VITE_MISTRAL_API_KEY;
...
[OUTPUT RULES - STRICT]
- NEVER use markdown code fences (\`\`\`) unless the user specifically requested a code snippet.
...
- Always combine natural Markdown text + rich blocks for best note-taking experience.
    ...
    **Important Rules for Mermaid:**
    ...
    - Do not put it inside <pre>, <code>, or add hljs spans.
        ...
        If the user intent is to make an image or picture (words like "generate or make image / picture etc", "make image or picture", "create visual", "cinematic", "illustration", etc.) or contetn intent is realted to a scene or depict / involve a scene:
...
const INACTIVITY_TIMEOUT = 120000;  // 2 minutes - Best balance
const HARD_TOTAL_TIMEOUT = 600000;  // 10 minutes max (safety)
...
export async function Modify_By_AI(Apply) {
    ...
    try {
        ...
        let selectedText = "";
        let mediaStore = [];
        let hasColumns = false; // <-- ADD THIS LINE
        ...
        if (from !== to) {
            ...
            // Extract & convert existing column structures into AI-safe plain markers
            const columnNodes = tempDiv.querySelectorAll('.columns');
            if (columnNodes.length > 0) {
                hasColumns = true;
                columnNodes.forEach((colNode) => {
                    const count = colNode.getAttribute('columncount') || '2';
                    const childCols = Array.from(colNode.querySelectorAll('.column'));

                    let markerHTML = `\n\nCOLUMNS_LAYOUT_START count=${count}\n`;
                    childCols.forEach((col, idx) => {
                        markerHTML += `COL_START_${idx + 1}\n${col.innerHTML}\nCOL_END_${idx + 1}\n`;
                    });
                    markerHTML += `COLUMNS_LAYOUT_END\n\n`;

                    const markerNode = document.createElement('div');
                    markerNode.innerHTML = markerHTML;
                    colNode.parentNode.replaceChild(markerNode, colNode);
                });
            }

            // Extract media and inject placeholders
            const mediaNodes = tempDiv.querySelectorAll('.override_media_position_in_live_editor');
            mediaNodes.forEach((node, index) => {
                const placeholder = `___MEDIA_BLOCK_${index}___`;

                // Save the exact HTML so TipTap can re-parse it later
                mediaStore.push({
                    placeholder: placeholder,
                    html: node.outerHTML
                });

                // Swap the media DOM node with the text placeholder
            ...
            // Prioritize HTML if media exists, otherwise fallback to Markdown
            if (editor.storage.markdown?.serialize && mediaStore.length === 0) selectedText = editor.storage.markdown.serialize(slice.content);
        ...
        const contentToSend = (mediaStore.length > 0 || hasColumns) ? selectedHTML : (selectedText || selectedHTML);
        ...
        try {
            ...
            let userInstruction = Apply.action === "Custom" ? prompt_input.value : Apply.content;

            if (mediaStore.length > 0) {
                userInstruction += `\n\n[CRITICAL FORMATTING RULE]: The provided content contains media placeholders formatted exactly as ___MEDIA_BLOCK_0___, ___MEDIA_BLOCK_1___, etc. You MUST preserve these exact placeholders in your generated output where they contextually belong. Do NOT remove them, do NOT alter them, and do NOT place them inside markdown code blocks.`;
            }

            if (hasColumns) {
                userInstruction += `\n\n[COLUMN FORMATTING RULE]: The content contains column layouts marked with COLUMNS_LAYOUT_START count=N, COL_START_1 ... COL_END_1, COLUMNS_LAYOUT_END. You MUST preserve this exact column structure. Modify the text inside the respective COL_START_x and COL_END_x markers as requested. Do NOT remove or modify these marker tags.`;
            }
                        ...
                        if (data.type === "message.output.delta" && data.content) {
                            ...
                            // Case 2: Normal text content inside stream loop
                                ...
                                renderContent = async (md) => {
                                    ...
                                    // Rebuild columns from markers back to Tiptap HTML
                                    if (hasColumns || htmlOutput.includes('COLUMNS_LAYOUT_START')) {
                                        htmlOutput = rebuildColumnsFromHTML(htmlOutput);
                                    }

                                    if (mediaStore.length > 0) {
                                        mediaStore.forEach((media, index) => {

                                            const regexP = new RegExp(
                                                `<p>\\s*<em>\\s*<strong>\\s*MEDIA_BLOCK_${index}\\s*</strong>\\s*</em>\\s*</p>`,
                                                'g'
                                            );

                                            // Optional fallback if it appears without the <p> wrapper
                                            const regexFallback = new RegExp(
                                                `<em>\\s*<strong>\\s*MEDIA_BLOCK_${index}\\s*</strong>\\s*</em>`,
                                                'g'
                                            );

                                            htmlOutput = htmlOutput.replace(regexP, media.html);
                                            htmlOutput = htmlOutput.replace(regexFallback, media.html);
                                        });
                                    }
...
// ==========================================
// COLUMN REBUILD UTILITY FOR TIPTAP (ROBUST)
// ==========================================
function rebuildColumnsFromHTML(htmlContent) {
    console.groupCollapsed("[COLUMN_REBUILD] AI HTML Processing");
    console.log("1. Original HTML Input:\n", htmlContent);

    let result = htmlContent;

    // STEP 1: FORCE BREAKOUT REPLACEMENTS
    // We explicitly inject </p> and <p> to force Tiptap out of Inline node schemas,
    // guaranteeing our Block-level columns are never trapped inside a paragraph.
    result = result.replace(/COLUMNS_LAYOUT_START\s+count=(\d+)/g, '</p><div columncount="$1" class="columns" style="display: flex; gap: 24px; width: 100%;">');

    result = result.replace(/COL_START_\d+/g, '<div class="column" style="min-width: 0px;"><p>');

    result = result.replace(/COL_END_\d+/g, '</p></div>');

    result = result.replace(/COLUMNS_LAYOUT_END/g, '</div><p>');

    // STEP 2: BOUNDARY CLEANUP
    // Safely strip <br>, newlines, and spaces immediately touching ANY <div> or </div>
    result = result.replace(/(?:<br\s*\/?>|\n|\s)*(<\/?div[^>]*>)(?:<br\s*\/?>|\n|\s)*/gi, '$1');

    // Safely strip <br>, newlines, and spaces immediately touching ANY <p> or </p>
    result = result.replace(/(?:<br\s*\/?>|\n|\s)*(<\/?p[^>]*>)(?:<br\s*\/?>|\n|\s)*/gi, '$1');

    // STEP 3: GARBAGE COLLECTION
    // Remove any stray empty paragraphs created by the breakout logic
    result = result.replace(/<p[^>]*>\s*<\/p>/gi, '');

    console.log("2. Final Rebuilt HTML Output:\n", result);
    console.groupEnd();
