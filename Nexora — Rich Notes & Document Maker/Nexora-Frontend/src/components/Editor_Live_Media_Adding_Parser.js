...
export function startResize(event, container, editor, direction, getPos) {
    ...
    Stop_Resize_Method_Reference = (e) => stopResize(container, editor, getPos);
...
function stopResize(container, editor, getPos) {
    if (!container) return;
    ...
    if (newWidth && newHeight && !isNaN(newWidth) && !isNaN(newHeight)) {
        // ✅ Safely resolve target node position
        if (typeof getPos === 'function') {
            ...
            if (pos !== undefined && pos !== null) {
                ...
                if (node && node.type.name === 'media') {
                    // Update ONLY this targeted node by position
...
export function startRotate(event, container, editor, getPos) {
    event.preventDefault();
...
function stopRotate() {
    ...
    const { editorRef, getPos } = Rotation_State;

    try {
        if (typeof getPos === 'function' && editorRef) {
            const pos = getPos();
            if (pos !== undefined && pos !== null) {
